<?php
session_start();

require '../../includes/requeriments_cafeteria.php';

if (!isset($_SESSION['id_usuario'])) {
    header('Location: ../../login.php');
    exit();
}

if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    session_destroy();
    header('Location: ../../login.php');
    exit();
}
?>

<!DOCTYPE html>
<html>
<head>
    <title>Registro de Productos</title>
    <link rel="stylesheet" href="../../css/cafeteria_ver.css">
</head>
<body>
    <div class="sidebar">
        <img class="image-logo"
         src="data:image/png;base64,"
         alt="Casa del Café">
        <a href="../index.php">Ir al menú principal</a>
        <a href="verVentas.php">Ver Ventas</a>
        <a href="verProductos.php">Ver Productos</a>
        <a href="verIngresos.php">Ver Ingresos</a>
        <form action="registroProductos.php" method="POST">
            <input type="submit" value="Cerrar Sesión" formnovalidate>
        </form>
    </div>
    <div class="content">
        <h1><a class="back-link" href="../index.php">&lt;Volver</a>Productos</h1>
        
        <div class="busqueda">
            <label for="buscador">Buscar: </label>
            <input type="text" name="buscador" id="buscador" onchange="busqueda()" placeholder="Ingrese valor a Buscar">
        </div>

        <?php 
            if($_SESSION['rol'] != "empleado"){
                echo '<button class="btnRegistro"><a href="registroInventario.php">Registro de Inventario</a></button>
                <button class="btnRegistro"><a href="registroProductos.php">Registro de Productos</a></button>';
            }
        ?>

        <table id="tablaProductos">
        <tr>
            <th>#</th>
            <th>Imagen</th>
            <th>Nombre del Producto</th>
            <th>Precio</th>
            <th>Cantidad disponible</th>
            <?php 
                if($_SESSION['rol'] != "empleado"){
                    echo '<th>Acciones</th>';
                }
            ?>
        </tr>

        <?php
            $productoDAO = new ProductoDAO();
            $productos = $productoDAO->obtener_productos();

            foreach ($productos as $producto) {
                $precioProducto = $producto->getPrecio();
                echo "<tr>";
                echo "<td>". $producto->getId(). "</td>";
                echo "<td><img src='data:image/jpeg;base64," . base64_encode($producto->getImagen()) . "' width='100' height='100' /></td>";
                echo "<td>" . $producto->getNom() . "</td>";
                echo "<td>" . "$ " . number_format($precioProducto, 2, ',', '.') . "</td>";
                echo "<td>" . $producto->getCantidad() . "</td>";
                if($_SESSION['rol'] != "empleado"){
                    echo '<td><button class="modificar acciones">Modificar</button></td>';
                }
                echo "</tr>";
            }
        ?>
    </table>
    </div>


    <div id="dialog" style="display:none;">
        <h2>Editar producto</h2>
        <div class="dialogSpace">
            <p>Cantidad:</p>
            <input type="text" id="editCantidad" placeholder="Cantidad">
        </div>
        <div class="dialogSpace">
            <p>Nuevo precio:</p>
            <input type="text" id="editPrecio" placeholder="Nuevo Precio">
        </div>
        <button id="btnGuardar">Guardar</button>
        <button id="btnCerrar">Cerrar</button>
    </div>

</body>

<script>

document.addEventListener("DOMContentLoaded", function() {
    var modificarBotones = document.querySelectorAll('button.modificar');
    modificarBotones.forEach(function (boton) {
        boton.addEventListener('click', function () {
            var fila = this.parentElement.parentElement;

            var idProducto = fila.querySelector('td:nth-child(1)').textContent; 
            var cantidadProducto = fila.querySelector('td:nth-child(5)').textContent;
            var precioProducto = fila.querySelector('td:nth-child(4)').textContent;

            document.getElementById('editCantidad').value = cantidadProducto;
            document.getElementById('editPrecio').value = precioProducto;

            document.getElementById('dialog').style.display = 'block';

            document.getElementById('btnGuardar').addEventListener('click', function () {
                
                var formData = new FormData();
                cantidadProducto = document.getElementById('editCantidad').value;
                precioProducto = document.getElementById('editPrecio').value;
                let id = idProducto;

                formData.append("idProducto", id);
                formData.append("cantidad", cantidadProducto);
                formData.append("precioProducto", precioProducto);
                
                var xhr = new XMLHttpRequest();
                xhr.open("POST", "modificarProducto.php", true);
                xhr.onreadystatechange = function() {
                    if (xhr.readyState === 4 && xhr.status === 200) {
                        alert(xhr.responseText);
                        window.location.reload();
                    }
                };
                xhr.send(formData);
            });

        });
    });
    document.getElementById('btnCerrar').addEventListener('click', function () {
        document.getElementById('dialog').style.display = 'none';
    });
});


</script>

</html>
