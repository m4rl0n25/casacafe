<?php
session_start();
require '../../includes/requeriments_administracion.php';

if (!isset($_SESSION['id_usuario'])) {
    header('Location: ../../login.php');
    exit();
}else if($_SESSION['rol'] != 'administrador'){
    header('Location: ../../home.php');
    exit();
}


if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    session_destroy();
    header('Location: ../../login.php');
    exit();
}


?>

<!DOCTYPE html>
<html>
<head>
    <title>Módulo de Administración</title>
    <title>Home</title>
    <link rel="stylesheet" href="../../css/administracion_ver_usuarios.css">
</head>
<body>
    <div class="sidebar">
        <img class="image-logo"
         src="data:image/png;base64,"
         alt="Casa del Café">
        <a href="../../home.php">Ir al menú principal</a>
        <a href="verUsuarios.php">Ver Usuarios</a>
        <a href="editarRoles.php">Editar Roles</a>
        <a href="parametrosRoles.php">Editar parametros cargos</a>
        <form action="verUsuarios.php" method="POST">
            <input type="submit" value="Cerrar Sesión" formnovalidate>
        </form>
    </div>
    <div class="content">
        <h1><a class="back-link" href="../index.php">&lt;Volver</a> Ver Usuarios</h1>
        <input type="text" id="inputBuscarNombre" placeholder="Buscar por Nombre">
        <button id="btnBuscarNombre" class="acciones"><b>Buscar</b></button>
        <button class="acciones"><a href="registro.php">Registrar Usuario</a></button><br><br>
        <div class="resultado_busqueda">
        <table id="tablaUsuarios">
            <thead>
                <th>Nombre</th>
                <th>Apellido</th>
                <th>Rol</th>
                <th>Email</th>
                <th>Acciones</th>
            </thead>
            <tbody>
            <?php
            $usuarioDAO = new UsuarioDAO();
            $usuarios = $usuarioDAO->obtener_todos_usuarios(); 

            foreach ($usuarios as $usuario) {
                if($usuario->get_usuario_activo() == 1){
                    echo '<tr data="' . $usuario->obtenerAtributosSeparadosPorPipe() . '">';
                    echo "<td>" . $usuario->get_nombre() . "</td>";
                    echo "<td>" . $usuario->get_apellido() . "</td>";
                    echo "<td>" . $usuario->obtener_rol() . "</td>";
                    echo "<td>" . $usuario->get_email() . "</td>";
                    echo '<td>';
                    echo '<button class="btnMostrar acciones">Mostrar</button>';
                    echo '<button class="btnModificar acciones">Modificar</button>';
                    echo '<button class="btnEliminar acciones">Eliminar</button>';
                    echo '</td>';
                    echo "</tr>";
                }
            }
            ?>
            </tbody>
        </table>
        </div>
    </div>
</body>

<script>
    document.addEventListener("DOMContentLoaded", function() {
    var table = document.getElementById("tablaUsuarios");
    var rows = table.getElementsByTagName("tr");
    var dialogOverlay = document.querySelector(".dialog-overlay");
    var dialogBox = document.querySelector(".dialog-box");
    var editDialog = document.querySelector(".edit-dialog");
    var editNombreInput = document.getElementById("editNombre");
    var editApellidoInput = document.getElementById("editApellido");
    var editCorreoInput = document.getElementById("editCorreo");
    var btnGuardar = document.getElementById("btnGuardar");
    var btnCerrar = document.getElementById("btnCerrar");
    var currentlyEditingUserId = null;
    var inputBuscarNombre = document.getElementById("inputBuscarNombre");
    var btnBuscarNombre = document.getElementById("btnBuscarNombre");
    var resultadoBusqueda = document.getElementById("resultadoBusqueda");

    btnBuscarNombre.addEventListener("click", function() {
        var nombreABuscar = inputBuscarNombre.value.toLowerCase();
        var filas = document.getElementById("tablaUsuarios").getElementsByTagName("tbody")[0].getElementsByTagName("tr");

        for (var i = 0; i < filas.length; i++) {
            var nombreEnFila = filas[i].getElementsByTagName("td")[0].innerText.toLowerCase();

            if (nombreEnFila.includes(nombreABuscar)) {
                filas[i].style.display = "";
            } else {
                filas[i].style.display = "none";
            }
        }
    });

    inputBuscarNombre.addEventListener("input", function() {
        var nombreABuscar = inputBuscarNombre.value.toLowerCase();
        var filas = document.getElementById("tablaUsuarios").getElementsByTagName("tbody")[0].getElementsByTagName("tr");
        
        resultadoBusqueda.innerHTML = "";

        for (var i = 0; i < filas.length; i++) {
            var nombreEnFila = filas[i].getElementsByTagName("td")[0].innerText.toLowerCase();

            if (nombreEnFila.includes(nombreABuscar)) {
                filas[i].style.display = "";
                resultadoBusqueda.innerHTML += "Resultado: " + filas[i].innerText + "<br>";
            } else {
                filas[i].style.display = "none";
            }
        }
    });

    for (var i = 1; i < rows.length; i++) {
        var row = rows[i];
        var data = row.getAttribute("data");
        var buttons = row.getElementsByTagName("button");

        buttons[0].addEventListener("click", function(data) {
            return function() {
                dialogOverlay.style.display = "block";
                dialogBox.innerHTML = "Datos del usuario:<br>";
                var usuarioData = data.split("|");
                dialogBox.innerHTML += "ID: " + usuarioData[0] + "<br>";
                dialogBox.innerHTML += "Nombre: " + usuarioData[1] + "<br>";
                dialogBox.innerHTML += "Apellido: " + usuarioData[2] + "<br>";
                dialogBox.innerHTML += "Rol: " + usuarioData[3] + "<br>";
                dialogBox.innerHTML += "Email: " + usuarioData[4] + "<br>";

                dialogBox.innerHTML += "<button id='cerrarDialog'>Cerrar</button>";

                var cerrarDialog = dialogBox.querySelector("#cerrarDialog");
                cerrarDialog.addEventListener("click", function() {
                    dialogOverlay.style.display = "none";
                });
            };
        }(data));

       buttons[1].addEventListener("click", function(data) {
            return function() {
                var usuarioData = data.split("|");
                var userId = usuarioData[0];

                editNombreInput.value = usuarioData[1];
                editApellidoInput.value = usuarioData[2];
                editCorreoInput.value = usuarioData[4];
                currentlyEditingUserId = userId;
                editDialog.style.display = "block";
            };
        }(data));

        buttons[2].addEventListener("click", function(data) {
            return function() {
                if (confirm("¿Estás seguro de que deseas eliminar este usuario?")) {
                    var usuarioData = data.split("|");
                    var userId = usuarioData[0];

                    var xhr = new XMLHttpRequest();
                    xhr.open("POST", "eliminarUsuario.php", true);
                    xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
                    xhr.onreadystatechange = function() {
                        if (xhr.readyState === 4 && xhr.status === 200) {
                            alert(xhr.responseText);
                            window.location.reload();
                        }
                    };
                    xhr.send("user_id=" + userId);
                }
            };
        }(data));
    }

    btnGuardar.addEventListener("click", function() {
        if (currentlyEditingUserId) {
            var newNombre = editNombreInput.value;
            var newApellido = editApellido.value;
            var newCorreo = editCorreoInput.value;

            var formData = new FormData();
            formData.append("userID", currentlyEditingUserId);
            formData.append("newNombre", newNombre);
            formData.append("newApellido", newApellido);
            formData.append("newCorreo", newCorreo);

            var xhr = new XMLHttpRequest();
            xhr.open("POST", "modificarUsuario.php", true);
            xhr.onreadystatechange = function() {
                if (xhr.readyState === 4 && xhr.status === 200) {
                    alert(xhr.responseText);
                    window.location.reload();
                }
            };
            xhr.send(formData);

            editDialog.style.display = "none";
        }
    });

    btnCerrar.addEventListener("click", function() {
        editDialog.style.display = "none";
    });

});



</script>

<div class="dialog-overlay">
    <div class="dialog-box">
        
    </div>
</div>

<div class="edit-dialog" style="display:none;">
    <h2>Editar Usuario</h2>
    <div class="dialogSpace">
        <p>Nuevo nombre:</p>
        <input type="text" id="editNombre" placeholder="Nuevo Nombre">
    </div>
    <div class="dialogSpace">
        <p>Nuevo Apellido:</p>
        <input type="text" id="editApellido" placeholder="Nuevo Apellido">
    </div>
    <div class="dialogSpace">
        <p>Nuevo Correo:</p>
        <input type="text" id="editCorreo" placeholder="Nuevo Correo">
    </div>
    <button id="btnGuardar">Guardar</button>
    <button id="btnCerrar">Cerrar</button>
</div>


</html>

