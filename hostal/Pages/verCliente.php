<?php
session_start();
require '../../includes/requeriments_hospedaje.php';

if (!isset($_SESSION['id_usuario'])) {
    header('Location: ../../login.php');
    exit();
}

if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    session_destroy();
    header('Location: ../../login.php');
    exit();
}
?>

<!DOCTYPE html>
<html>
<head>
    <title>Módulo de Administración</title>
    <link rel="stylesheet" href="../../css/hotas_ver_datos.css">
</head>
<body>
<div class="sidebar">
        <img class="image-logo"
         src="data:image/png;base64,"
         alt="Casa del Café">
        <a href="../index.php">Ir al menú principal</a>
        <a href="verCliente.php">Ver Clientes</a>
        <a href="habitaciones.php">Ver Habitaciones</a>
        <a href="verReservas.php">Ver Reservas</a>
        <a href="registroIngresos.php">Registro de Ingresos</a>
        <form action="verCliente.php" method="POST">
            <input type="submit" value="Cerrar Sesión" formnovalidate>
        </form>
    </div>
    <div class="content">
        <h1><a class="back-link" href="../index.php">&lt;Volver</a> Clientes</h1>

        <div class="busqueda">
            <label for="buscador">Buscar: </label>
            <input type="text" name="buscador" id="buscador" onchange="busqueda()" placeholder="Ingrese valor a Buscar">
        </div>

        <?php 
            if($_SESSION['rol'] != "empleado"){
                echo '<button class="btnRegistro"><a href="registrarCliente.php">Registrar Cliente</a></button>';
            }
        ?>

        <table id="tablaClientes">
            <thead>
                <th>ID Cliente</th>
                <th>Nombre</th>
                <th>Email</th>
                <th>Ubicacion</th>
                <th>Numero</th>
                <?php 
                    if($_SESSION['rol'] != "empleado"){
                        echo '<th>Acciones</th>';
                    }
                ?>
            </thead>
            <tbody>
            <?php
            $clienteDAO = new ClienteDAO();
            $clientes = $clienteDAO->obtenerClientes();

            foreach ($clientes as $cliente) {
                echo '<tr data="' . $cliente->obtenerAtributosSeparadosPorPipe() . '">';
                echo "<td>" . $cliente->getId() . "</td>";
                echo "<td>" . $cliente->getNombre() . "</td>";
                echo "<td>" . $cliente->getEmail() . "</td>";
                echo "<td>" . $cliente->getubicacion() . "</td>";
                echo "<td>" . $cliente->getNumero() . "</td>";
                if($_SESSION['rol'] != "empleado"){                 
                    echo '<td>';
                    echo '<button class="accion mostrar">Mostrar</button>';
                    echo '<button class="accion modificar">Modificar</button>';
                    echo '</td>';
                }
                echo '</tr>';
            }
            ?>
            </tbody>
        </table>
    </div>

    <div id="dialog" title="Detalles del Cliente" style="display:none;">
        <p><strong>ID Cliente:</strong> <span id="clienteId"></span></p>
        <p><strong>Nombre:</strong> <span id="clienteNombre"></span></p>
        <p><strong>Email:</strong> <span id="clienteEmail"></span></p>
        <button id='cerrarDialog'>Cerrar</button>
    </div>

    <div id="confirmarEliminarDialog" title="Confirmar Eliminación" style="display:none;">
        <p>¿Estás seguro de que deseas eliminar este cliente?</p>
        <button id="cerrarEliminarDialog">Cerrar</button>
        <button id="confirmar">Eliminar</button>
    </div>

    <div id="editDialog" style="display:none;">
        <h2>Editar Cliente</h2>
        <div class="dialogSpace">
            <p>Nuevo Nombre:</p>
            <input type="text" id="editNombre" placeholder="Nuevo Nombre">
        </div>
        <div class="dialogSpace">
            <p>Nuevo Correo:</p>
            <input type="text" id="editCorreo" placeholder="Nuevo Correo">
        </div>
        <button id="btnGuardarEdit">Guardar</button>
        <button id="btnCerrarEdit">Cerrar</button>
    </div>

</body>

<script>
    var mostrarBotones = document.querySelectorAll('button.mostrar');
    var modificarBotones = document.querySelectorAll('button.modificar');
    var eliminarBotones = document.querySelectorAll('button.eliminar');
    var confirmarEliminarDialog = document.getElementById('confirmarEliminarDialog');

    mostrarBotones.forEach(function (boton) {
        boton.addEventListener('click', function () {
            var fila = this.parentElement.parentElement;

            var idCliente = fila.querySelector('td:nth-child(1)').textContent;
            var nombreCliente = fila.querySelector('td:nth-child(2)').textContent;
            var emailCliente = fila.querySelector('td:nth-child(3)').textContent;

            document.getElementById('clienteId').textContent = idCliente;
            document.getElementById('clienteNombre').textContent = nombreCliente;
            document.getElementById('clienteEmail').textContent = emailCliente;

            document.getElementById('dialog').style.display = 'block';
        });
    });

    modificarBotones.forEach(function (boton) {
        boton.addEventListener('click', function () {
            var fila = this.parentElement.parentElement;

            var idCliente = fila.querySelector('td:nth-child(1)').textContent;
            var nombreCliente = fila.querySelector('td:nth-child(2)').textContent;
            var emailCliente = fila.querySelector('td:nth-child(3)').textContent;

            document.getElementById('editNombre').value = nombreCliente;
            document.getElementById('editCorreo').value = emailCliente;

            document.getElementById('editDialog').style.display = 'block';

            

            document.getElementById('btnGuardarEdit').addEventListener('click', function () {
                
                var formData = new FormData();
                nombreCliente = document.getElementById('editNombre').value;
                emailCliente = document.getElementById('editCorreo').value;

                formData.append("idCliente", idCliente);
                formData.append("nombreCliente", nombreCliente);
                formData.append("emailCliente", emailCliente);
                
                var xhr = new XMLHttpRequest();
                xhr.open("POST", "modificarCliente.php", true);
                xhr.onreadystatechange = function() {
                    if (xhr.readyState === 4 && xhr.status === 200) {
                        alert(xhr.responseText);
                        window.location.reload();
                    }
                };
                xhr.send(formData);
            });

        });
    });

    eliminarBotones.forEach(function (boton) {
        boton.addEventListener('click', function () {
            var fila = this.parentElement.parentElement;
            var idCliente = fila.querySelector('td:nth-child(1)').textContent;

            var confirmarEliminarDialog = document.getElementById('confirmarEliminarDialog');
            confirmarEliminarDialog.style.display = 'block';

            var formData = new FormData();
            formData.append("idCliente", idCliente);

            document.getElementById('confirmar').addEventListener('click', function () {
                alert('Eliminar cliente con ID: ' + idCliente);
                var xhr = new XMLHttpRequest();
                xhr.open("POST", "eliminarCliente.php", true);
                xhr.onreadystatechange = function() {
                    if (xhr.readyState === 4 && xhr.status === 200) {
                        alert(xhr.responseText);
                        window.location.reload();
                    }
                };
                xhr.send(formData);
            });
        });
    });

    document.getElementById('cerrarDialog').addEventListener('click', function () {
        var confirmarEliminarDialog = document.getElementById('dialog');
        confirmarEliminarDialog.style.display = 'none';
    });

    document.getElementById('btnCerrarEdit').addEventListener('click', function () {
        document.getElementById('editDialog').style.display = 'none';
    });

    function busqueda() {
        var input = document.getElementById('buscador');
        var filter = input.value.toUpperCase();

        var table = document.getElementById('tablaClientes');
        var rows = table.getElementsByTagName('tr');

        for (var i = 0; i < rows.length; i++) {
            var tdNombre = rows[i].getElementsByTagName('td')[1];
            if (tdNombre) {
                var txtValue = tdNombre.textContent || tdNombre.innerText;
                if (txtValue.toUpperCase().indexOf(filter) > -1) {
                    rows[i].style.display = '';
                } else {
                    rows[i].style.display = 'none';
                }
            }
        }
    }

</script>

</html>
