...
<body>
    ...
    <div class="content">
        <h1><a class="back-link" href="../index.php">&lt;Volver</a>Ventas</h1>
        ...
        <?php 
            if($_SESSION['rol'] != "empleado"){
                echo '<button class="btnRegistro"><a href="registroVentas.php">Registro de Ventas</a></button>';
            }
        ?>
        ...
        <?php
            ...
            $ventas = $ventasDAO->obtener_todas_ventas();
            $productoDAO = new ProductoDAO();

            foreach ($ventas as $venta) {
                $producto = $productoDAO->obtener_producto_por_id($venta->get_id_producto());
                $precioProducto = $producto->getPrecio();
                $precioTotal = $venta->get_cantidad() * $precioProducto;

                echo "<tr>";
                echo "<td>" . $venta->get_id_venta() . "</td>";
                echo "<td>" . $venta->get_fecha() . "</td>";
                echo "<td>" . $producto->getNom(). " #". $producto->getId() . "</td>";
                echo "<td>" . $venta->get_cantidad() . "</td>";
...
<script>
    function busqueda() {
        ...
        for (var i = 0; i < rows.length; i++) {
            var tdNombre = rows[i].getElementsByTagName('td')[2];
            if (tdNombre) {
