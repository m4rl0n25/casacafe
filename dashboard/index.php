<?php
session_start();

require '../includes/requeriments_dashboard.php';

$reservaDAO = new ReservaDAO();
$datosGrafica = $reservaDAO->obtenerIngresosPorSemana();
$ingresosMesAnterior = $reservaDAO->obtenerIngresosMesActual();
$ingresosAnioActual = $reservaDAO->obtenerIngresosAnioActual();
$graficaDataReserva = array();

if (!empty($datosGrafica)) {
    foreach($datosGrafica as $dato) {
        $semana = (int) $dato['semana'];
        $ingresos = (float) $dato['ingresos'];
        
        $dataPoint = array(
            'semana' => $semana,
            'ingresos' => $ingresos
        );

        $graficaDataReserva[] = $dataPoint;
    }
}

$graficaReservaDataJSON = json_encode($graficaDataReserva);


$ventasDAO = new VentaDAO();
$ventasMesActual = $ventasDAO->obtenerIngresosMesActual();
$ventasAnioActual = $ventasDAO->obtenerIngresosAnioActual();
$datosGrafica = $ventasDAO->obtener_ingresos_por_semana();
$graficaData = array();

if (!empty($datosGrafica)) {
    foreach($datosGrafica as $dato) {
        $semana = (int) $dato['semana'];
        $ingresos = (float) $dato['ingresos'];
        
        $dataPoint = array(
            'semana' => $semana,
            'ingresos' => $ingresos
        );

        $graficaData[] = $dataPoint;
    }
}

$graficaDataJSON = json_encode($graficaData);


if (!isset($_SESSION['id_usuario'])) {
    header('Location: ../login.php');
    exit();
}

if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    session_destroy();
    header('Location: ../login.php');
    exit();
}
?>

<!DOCTYPE html>
<html>
<head>
    <title>Ver Ingresos</title>
    <link rel="stylesheet" href="../css/dash_index.css">
    <script src="https://cdnjs.cloudflare.com/ajax/libs/Chart.js/3.7.0/chart.min.js"></script>
</head>
<body>
    <div class="sidebar">
        <img class="image-logo"
         src="data:image/png;base64,"
         alt="Casa del Café">
        <a href="../home.php">Ir al menú principal</a>
        <form action="registroProductos.php" method="POST">
            <input type="submit" value="Cerrar Sesión" formnovalidate>
        </form>
    </div>
    <div class="content">
        <h1><a class="back-link" href="../index.php">&lt;Volver</a>Ver Ingresos</h1>

        <div class="burbujas">
            <div class="burbujasVentas">
                <h2>Ingresos Ventas</h2>
                <div class="containerDatos">
                    <div class="burbuja">
                        <div class="datos">
                            <p class="datoValue"><?php echo $ventasAnioActual ?></p>
                            <p class="datoPrin">Ingresos año</p>
                        </div>
                    </div>
                    <div class="burbuja">
                        <div class="datos">
                            <p class="datoValue"><?php echo $ventasMesActual ?></p>
                            <p class="datoPrin">Ingresos Mes</p>
                        </div>
                    </div>
                </div>
            </div>
            <div class="burbujasVentas">
                <h2>Ingresos Reservas</h2>
                <div class="containerDatos">
                    <div class="burbuja">
                        <div class="datos">
                            <p class="datoValue"><?php echo $ingresosAnioActual ?></p>
                            <p class="datoPrin">Ingresos año</p>
                        </div>
                    </div>
                    <div class="burbuja">
                        <div class="datos">
                            <p class="datoValue"><?php echo $ingresosMesAnterior ?></p>
                            <p class="datoPrin">Ingresos Mes</p>
                        </div>
                    </div>
                </div>
            </div>
        </div>

        <div class="graficas">
            <div class="grafica-container">
                <h2>Ingresos Ventas</h2>
                <canvas id="graficaIngresosVentas"></canvas>
            </div>
            <div class="grafica-container">
                <h2>Ingresos Reservas</h2>
                <canvas id="graficaIngresosReservas"></canvas>
            </div>
        </div>
    </div>
</body>

<script>
    var datosGrafica = <?php echo $graficaDataJSON; ?>;
    var datosGraficaReserva = <?php echo $graficaReservaDataJSON; ?>;

    var semanas = datosGrafica.map(function (dato) {
        return dato.semana;
    });

    var ingresos = datosGrafica.map(function (dato) {
        return dato.ingresos;
    });

    var semanasReserva = datosGraficaReserva.map(function (dato) {
        return dato.semana;
    });

    var ingresosReserva = datosGraficaReserva.map(function (dato) {
        return dato.ingresos;
    });

    var ctx = document.getElementById('graficaIngresosVentas').getContext('2d');
    var graficaIngresos = new Chart(ctx, {
        type: 'bar',
        data: {
            labels: semanas, // Usamos las semanas
            datasets: [{
                label: 'Ingresos por Semana',
                data: ingresos, // Usamos los ingresos
                backgroundColor: 'rgba(75, 192, 192, 0.2)', // Color de las barras
                borderColor: 'rgba(75, 192, 192, 1)', // Color del borde de las barras
                borderWidth: 1
            }]
        },
        options: {
            scales: {
                y: {
                    beginAtZero: true
                }
            }
        }
    });

    var ctx = document.getElementById('graficaIngresosReservas').getContext('2d');
    var graficaIngresos = new Chart(ctx, {
        type: 'bar',
        data: {
            labels: semanasReserva, // Usamos las semanas
            datasets: [{
                label: 'Ingresos por Semana',
                data: ingresosReserva, // Usamos los ingresos
                backgroundColor: 'rgba(75, 192, 192, 0.2)', // Color de las barras
                borderColor: 'rgba(75, 192, 192, 1)', // Color del borde de las barras
                borderWidth: 1
            }]
        },
        options: {
            scales: {
                y: {
                    beginAtZero: true
                }
            }
        }
    });
</script>

</html>
