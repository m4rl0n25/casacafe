<?php
session_start();
require '../../includes/requeriments_hospedaje.php';

if (!isset($_SESSION['id_usuario'])) {
    header('Location: ../../login.php');
    exit();
}

if($_SERVER['REQUEST_METHOD'] == 'POST'){
    session_destroy();
    header('Location: ../../login.php');
    exit();
}
?>

<!DOCTYPE html>
<html>
<head>
    <title>Habitaciones</title>
    <link rel="stylesheet" href="../../css/hostal_index.css">
</head>
<body>
    <div class="sidebar">
        <img class="image-logo"
         src="data:image/png;base64,"
         alt="Casa del Café">
        <a href="../../home.php">Ir al menú principal</a>
        <a href="verCliente.php">Ver Clientes</a>
        <a href="habitaciones.php">Ver Habitaciones</a>
        <a href="verReservas.php">Ver Reservas</a>
        <a href="registroIngresos.php">Registro de Ingresos</a>
        <form action="habitaciones.php" method="POST">
            <input type="submit" value="Cerrar Sesión" formnovalidate>
        </form>
    </div>
    <div class="content">
        <h1><a class="back-link" href="../index.php">&lt;Volver</a>Habitaciones</h1>
        <div class="options">
            <button><a href="registrarHabitacion.php">Registrar habitacion</a></button>
            <button><a href="verHabitacionesDisponibles.php">Habitaciones disponibles</a></button>
            <button><a href="verHabitacionesOcupadas.php">Habitaciones ocupadas</a></button>
            <button><a href="verHabitacionesReservadas.php">Habitaciones reservadas</a></button>
            <br><br>
            <button><a href="registrarCheckIN.php">Check In</a></button>
            <button><a href="registrarCheckOUT.php">Check out</a></button>
        </div>

        <br>

        <div class="buscador">
            <label for="buscador">Buscar: </label>
            <select name="buscador" id="buscador" onchange="busqueda()">
                <option value="All">Todos</option>
                <option value="Disponible">Disponibles</option>
                <option value="Ocupada">Ocupadas</option>
                <option value="Reservada">Reservada</option>
            </select>
        </div>

        <table id="tablaHabitaciones">
            <thead>
                <th>Numero Habitación</th>
                <th>Descripcion</th>
                <th>Estado</th>
            </thead>
            <tbody>
            <?php

            $habitacionDAO = new HabitacionDAO();
            $habitaciones = $habitacionDAO->obtenerHabitaciones();
            $reservaDAO = new ReservaDAO();

            foreach ($habitaciones as $habitacion) {
                
                $reserva = $reservaDAO->buscar_reserva_por_num_habitacion($habitacion->getIdHabitacion());
                if($reserva){
                    $isReservado = $reservaDAO->comprobar_reserva($reserva->getID());
                }else{
                    $isReservado = false;
                }

                if($isReservado){
                    echo '<tr class="reservada">';
                    echo "<td>" . $habitacion->getIdHabitacion() . "</td>";
                    echo "<td>" . $habitacion->getTipoHabitacion(). " para ". $habitacion->getCapacidad(). " personas" . "</td>";
                    echo "<td>" . "Reservada" . "</td>";
                }else{
                    if($habitacion->getdisponible() == 1){
                        echo '<tr class="disponible">';
                    }else{
                        echo '<tr class="ocupada">';
                    }
                    echo "<td>" . $habitacion->getIdHabitacion() . "</td>";
                    echo "<td>" . $habitacion->getTipoHabitacion(). " para ". $habitacion->getCapacidad(). " personas" . "</td>";
                    if($habitacion->getdisponible() == 1){
                        echo "<td>" . "Disponible" . "</td>";
                    }else{
                        echo "<td>" . "Ocupada" . "</td>";
                    }
                }
                
                echo "</tr>";

            }

            ?>
            </tbody>
        </table>
    </div>
</body>

<script>
    function busqueda(){
        var seleccion = document.getElementById("buscador").value;
        var filas = document.querySelectorAll("#tablaHabitaciones tbody tr");

        filas.forEach(function(fila) {
            var estado = fila.querySelector("td:nth-child(3)").textContent;

            if(seleccion === "All" || estado === seleccion) {
                fila.style.display = "table-row"; // Mostrar la fila
            } else {
                fila.style.display = "none"; // Ocultar la fila
            }
        });
}
</script>

</html>
