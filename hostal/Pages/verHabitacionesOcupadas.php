<?php
session_start();
require '../../includes/requeriments_hospedaje.php';

if (!isset($_SESSION['id_usuario'])) {
    header('Location: ../../login.php');
    exit();
}

if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    session_destroy();
    header('Location: ../../login.php');
    exit();
}

?>
<!DOCTYPE html>
<html>
<head>
    <title>Módulo de Hostal</title>
    <link rel="stylesheet" href="../../css/hotas_ver_datos.css">
</head>
<body>
    <div class="sidebar">
        <img class="image-logo"
         src="data:image/png;base64,"
         alt="Casa del Café">
         <a href="../index.php">Ir al menú principal</a>
        <a href="verCliente.php">Ver Clientes</a>
        <a href="habitaciones.php">Ver Habitaciones</a>
        <a href="verReservas.php">Ver Reservas</a>
        <a href="registroIngresos.php">Registro de Ingresos</a>
        <form action="verHabitacionesOcupadas.php" method="POST">
            <input type="submit" value="Cerrar Sesión" formnovalidate>
        </form>
    </div>
    <div class="content">
        <h1><a class="back-link" href="habitaciones.php">&lt;Volver</a> Habitaciones Ocupadas</h1>
        <table id="tablaHabitaciones">
            <thead>
                <th>Numero Habitación</th>
                <th>Tipo de Habitación</th>
                <th>Precio Día</th>
                <th>Precio Noche</th>
                <th>Nombre Cliente</th>
                <th>Email Cliente</th>
                <th>Capacidad habitacion</th>
                <th>Cantidad personas</th>
                <th>Estado</th>
                <?php 
                    if($_SESSION['rol'] != "empleado"){
                        echo '<th>Acciones</th>';
                    }
                ?>
            </thead>
            <tbody>
            <?php

            $habitacionDAO = new HabitacionDAO();
            $habitacionesOcupadas = $habitacionDAO->obtenerHabitacionesOcupadas();
            $clienteDAO = new ClienteDAO();
            $reservaDAO = new ReservaDAO();
            
            foreach ($habitacionesOcupadas as $habitacion) {
                $reserva = $reservaDAO->buscar_cantidad_por_id_cliente($habitacion->getIdCliente(), $habitacion->getIdHabitacion());
                $cliente = $clienteDAO->obtenerClientePorId($habitacion->getIdCliente());
                
                if($cliente){
                    $nombreCliente = $cliente->getNombre();
                    $email = $cliente->getEmail();
                }else{
                    $nombreCliente = "Desconocido";
                    $email = 'Desconocido';
                }
                
                if($reserva){
                    $cantidadPersonas = $reserva->getCantidadPersonas();
                }else {
                    $cantidadPersonas = 0;
                }

                $data = $habitacion->getIdHabitacion(). 
                "|". $habitacion->getTipoHabitacion(). 
                "|". $habitacion->getPrecioDia().
                "|". $habitacion->getPrecioNoche().
                "|". $email.
                "|". $habitacion->getCapacidad();

                echo '<tr data="'. $data.'">';
                echo "<td>" . $habitacion->getIdHabitacion() . "</td>";
                echo "<td>" . $habitacion->getTipoHabitacion() . "</td>";
                echo "<td>" . $habitacion->getPrecioDia() . "</td>";
                echo "<td>" . $habitacion->getPrecioNoche() . "</td>";
                echo "<td>" . $nombreCliente . "</td>";
                echo "<td>" . $email . "</td>";
                echo "<td>" . $habitacion->getCapacidad() . "</td>";
                echo "<td>" . $cantidadPersonas . "</td>";
                if($habitacion->getdisponible() == 0){
                   echo "<td class='SinReserva'>Check In</td>"; 
                }else{
                    echo "<td class='SinCheckIn'>Check Out</td>";
                }
                if($_SESSION['rol'] != "empleado"){
                    echo '<td>';
                    echo '<button class="btnMostrar acciones">Mostrar</button>';
                    echo '</td>';
                }
                echo "</tr>";
            
            }
            
            ?>
            </tbody>
        </table>
    </div>

    <div id="dialogMostrar" class="dialogs" style="display:none;">
        <h2>Habitacion ocupada</h2>
        <p><strong>Numero de habitacion #</strong><span></span></p>
        <p><strong>Tipo de habitacion: </strong><span></span></p>
        <p><strong>Precio dia: </strong><span></span></p>
        <p><strong>Precio noche: </strong><span></span></p>
        <p><strong>Capacidad habitacion: </strong><span></span></p>
        <p><strong>Cantidad personas: </strong><span></span></p>
        <button id="btnCerrarDialogMostrar">Cerrar</button>
    </div>

</body>

<script>
    document.addEventListener("DOMContentLoaded", function(){

        var mostrarDialog = document.getElementById("dialogMostrar");

        var rows = document.getElementsByTagName("tr");

        for(let index = 1; index < rows.length; index++){
            let row = rows[index];
            let data = row.getAttribute("data");
            let button = row.querySelector("button"); 

            button.addEventListener("click", function(data){
                return function(){
                    let rowData = data.split("|");
                    let spans = mostrarDialog.getElementsByTagName("span");
                    
                    for(let index = 0; index < spans.length; index++){
                        spans[index].textContent = rowData[index];
                    }
                    mostrarDialog.style.display = "block";
                };
            }(data));
        }

        document.getElementById("btnCerrarDialogMostrar").addEventListener("click", function(){
            mostrarDialog.style.display = "none";
        });

    });

</script>

</html>
