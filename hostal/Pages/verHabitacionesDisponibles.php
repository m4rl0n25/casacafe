<?php
session_start();
require '../../includes/requeriments_hospedaje.php';

if (!isset($_SESSION['id_usuario'])) {
    header('Location: ../../login.php');
    exit();
}

if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    session_destroy();
    header('Location: ../../login.php');
    exit();
}
?>

<!DOCTYPE html>
<html>
<head>
    <title>Módulo de Hostal</title>
    <link rel="stylesheet" href="../../css/hotas_ver_datos.css">
</head>
<body>
    <div class="sidebar">
        <img class="image-logo"
         src="data:image/png;base64,"
         alt="Casa del Café">
         <a href="../index.php">Ir al menú principal</a>
        <a href="verCliente.php">Ver Clientes</a>
        <a href="habitaciones.php">Ver Habitaciones</a>
        <a href="verReservas.php">Ver Reservas</a>
        <a href="registroIngresos.php">Registro de Ingresos</a>
        <form action="verHabitacionesDisponibles.php" method="POST">
            <input type="submit" value="Cerrar Sesión" formnovalidate>
        </form>
    </div>
    <div class="content">
        <h1><a class="back-link" href="habitaciones.php">&lt;Volver</a> Habitaciones Disponibles</h1>
        <table id="tablaHabitaciones">
            <thead>
                <th>Numero Habitación</th>
                <th>Tipo de Habitación</th>
                <th>Capacidad</th>
                <th>Precio Día</th>
                <th>Precio Noche</th>
            </thead>
            <tbody>
            <?php
            $habitacionDAO = new HabitacionDAO();
            $habitacionesDisponibles = $habitacionDAO->obtenerHabitacionesDisponibles();
            $reservaDAO = new ReservaDAO();

            foreach ($habitacionesDisponibles as $habitacion) {
                $reserva = $reservaDAO->buscar_reserva_por_num_habitacion($habitacion->getIdHabitacion());
                echo "<tr data='".$habitacion->getIdHabitacion(). "|". $habitacion->getTipoHabitacion() ."'>";
                echo "<td>" . $habitacion->getIdHabitacion() . "</td>";
                echo "<td>" . $habitacion->getTipoHabitacion() . "</td>";
                echo "<td>" . $habitacion->getCapacidad(). " Personas" . "</td>";
                echo "<td>" . $habitacion->getPrecioDia() . "</td>";
                echo "<td>" . $habitacion->getPrecioNoche() . "</td>";
                echo "</tr>";
            }
            ?>
            </tbody>
        </table>
    </div>
</body>

<div id="dialogEliminar" class="dialogs" style="display:none;">
    <h2>Habitacion</h2>
    <p><strong>Numero de habitacion #</strong><span></span></p>
    <button id="btnEliminar">Eliminar</button>
    <button id="btnCerrarDialogEliminar">Cerrar</button>
</div>

<script>
    document.addEventListener("DOMContentLoaded", function(){

        var eliminarDialog = document.getElementById("dialogEliminar");

        var rows = document.getElementsByTagName("tr");
        
        for(let index=1; index < rows.length; index++){
            let row = rows[index];
            let data = row.getAttribute("data");
            let button = row.getElementsByTagName("button"); 
            
            button[0].addEventListener("click", function(data){
                return function(){
                    let rowData = data.split("|");
                    let spans = eliminarDialog.getElementsByTagName("span");
                
                    spans[0].textContent = rowData[0];
                    
                    eliminarDialog.style.display = "block";
            
                    document.getElementById("btnEliminar").addEventListener("click", function(){
                    var formData = new FormData();

                    formData.append("data", rowData);

                    var xhr = new XMLHttpRequest();
                    xhr.open("POST", "eliminarHabitacionService.php", true);
                        xhr.onreadystatechange = function(){
                            if(xhr.readyState == 4 & xhr.status == 200){
                                alert(xhr.responseText);
                                window.location.reload();
                            }
                        };
                        xhr.send(formData);
                    });

                };
            }(data));

            document.getElementById("btnCerrarDialogEliminar").addEventListener("click", function(){
                eliminarDialog.style.display = "none";
            });

        }
        
    });
</script>

</html>
