<?php
    session_start();
    require '../../includes/requeriments_hospedaje.php';

    if (!isset($_SESSION['id_usuario'])) {
        header('Location: ../../login.php');
        exit();
    }

    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        session_destroy();
        header('Location: ../../login.php');
        exit();
    }
?>

<!DOCTYPE html>
<html>
<head>
    <title>Registro de Reservas</title>
    <link rel="stylesheet" href="../../css/hotas_registrar.css">
</head>
<body>
<div class="sidebar">
        <img class="image-logo"
         src="data:image/png;base64,"
         alt="Casa del Café">
         <a href="../index.php">Ir al menú principal</a>
        <a href="verCliente.php">Ver Clientes</a>
        <a href="verHabitacionesDisponibles.php">Ver Habitaciones Disponibles</a>
        <a href="verHabitacionesOcupadas.php">Ver Habitaciones Ocupadas</a>
        <a href="verReservas.php">Ver Reservas</a>
        <a href="registroIngresos.php">Registro de Ingresos</a>
        <form action="registrarReserva.php" method="POST">
            <input type="submit" value="Cerrar Sesión" formnovalidate>
        </form>
    </div>
    <div class="content">
        <h1><a class="back-link" href="verReservas.php">&lt;Volver</a> Registro de Reservas</h1>
        <form class="formRegistro">
            <label for="tipo_habitacion">Tipo de Habitación:</label>
            <select name="tipo_habitacion" id="tipo_habitacion" onchange="descripcionCantidad(this)" required>
            <?php 
                $habitacionDAO = new HabitacionDAO();
                $habitaciones = $habitacionDAO->obtenerHabitacionesDisponibles();
                foreach ($habitaciones as $habitacion) {
                    echo "<option value='". $habitacion->getIdHabitacion(). "'>". $habitacion->getTipoHabitacion(). " #". $habitacion->getIdHabitacion(). "</option>";
                }
            ?>
            </select>

            <label for="descripcionHabitacion">Descripcion:</label>
            <input type="text" name="descripcionHabitacion" id="descripcionHabitacion" disabled>

            <label for="cliente">Cliente:</label>
            <select name="cliente" id="cliente" required>
            <?php 
                $clienteDAO = new ClienteDAO();
                $clientes = $clienteDAO->obtenerClientes();
                foreach ($clientes as $cliente) {
                    echo "<option value='". $cliente->getId(). "'>". $cliente->getEmail(). "</option>";
                }
            ?>
            </select>

            <label for="cantidadPersonas">Cantidad Personas:</label>
            <input type="text" name="cantidadPersonas" id="cantidadPersonas" required>

            <label for="fecha_inicio">Fecha de Inicio:</label>
            <input type="date" name="fecha_inicio" id="fecha_inicio" required>

            <label for="fecha_finalizacion">Fecha de Finalización:</label>
            <input type="date" name="fecha_finalizacion" id="fecha_finalizacion" required>
            
            <br><br>

            <input class="registroSumbit" type="submit" value="Registrar">
        </form>
    </div>
</body>

<script>

    function descripcionCantidad(select){
        var formData = new FormData();
        formData.append("id_habitacion", select.value);

        var descripcion = document.getElementById("descripcionHabitacion");
        var xhr = new XMLHttpRequest();
            xhr.open("POST", "habitacionService.php", true);
            xhr.onreadystatechange = function() {
                if (xhr.readyState === 4 && xhr.status === 200) {
                    var respuesta = JSON.parse(xhr.responseText);
                    
                    descripcion.value = respuesta.capacidad;
                }
            };
            xhr.send(formData);
    }

    document.addEventListener("DOMContentLoaded", function() {
        var formularioReserva = document.querySelector(".formRegistro"); // Cambiado a .formRegistro
        formularioReserva.addEventListener("submit", function(event) {
            event.preventDefault();

            var tipoHabitacion = document.getElementById("tipo_habitacion").value; // El id del select en el HTML es "tipo_habitacion"
            var cliente = document.getElementById("cliente").value;
            var fechaInicio = document.getElementById("fecha_inicio").value;
            var fechaFinalizacion = document.getElementById("fecha_finalizacion").value;
            var cantidadPersonas = document.getElementById("cantidadPersonas").value;

            var ayer = new Date();
            ayer.setDate(ayer.getDate() - 2);
            var fechaInicioDate = new Date(fechaInicio);
            var fechaFinalDate = new Date(fechaFinalizacion);

            if (fechaInicioDate <= ayer) {
                alert("La fecha de inicio no puede ser anterior a la fecha actual.");
                return;
            }

            if(fechaInicioDate > fechaFinalDate){
                alert("La fecha de finalizacion no puede ser menor a la fecha de inicio");
                return;
            }

            var formData = new FormData();
            formData.append("tipo_habitacion", tipoHabitacion);
            formData.append("cliente", cliente);
            formData.append("fecha_inicio", fechaInicio);
            formData.append("fecha_finalizacion", fechaFinalizacion);
            formData.append("cantidadPersonas", cantidadPersonas);

            var xhr = new XMLHttpRequest();
            xhr.open("POST", "registrarReservaService.php", true);
            xhr.onreadystatechange = function() {
                if (xhr.readyState === 4 && xhr.status === 200) {
                    alert(xhr.responseText);
                    window.location.reload();
                }
            };
            xhr.send(formData);
        });
    });
</script>

</html>
