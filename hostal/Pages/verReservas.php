<?php
session_start();
require '../../includes/requeriments_hospedaje.php';

if (!isset($_SESSION['id_usuario'])) {
    header('Location: ../../login.php');
    exit();
}

if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    session_destroy();
    header('Location: ../../login.php');
    exit();
}
?>

<!DOCTYPE html>
<html>
<head>
    <title>Reservas</title>
    <link rel="stylesheet" href="../../css/hotas_ver_datos.css">
</head>
<body>
<div class="sidebar">
        <img class="image-logo"
         src="data:image/png;base64,"
         alt="Casa del Café">
        <a href="../index.php">Ir al menú principal</a>
        <a href="verCliente.php">Ver Clientes</a>
        <a href="habitaciones.php">Ver Habitaciones</a>
        <a href="verReservas.php">Ver Reservas</a>
        <a href="registroIngresos.php">Registro de Ingresos</a>
        <form action="verReservas.php" method="POST">
            <input type="submit" value="Cerrar Sesión" formnovalidate>
        </form>
    </div>
    <div class="content">
        <h1><a class="back-link" href="../index.php">&lt;Volver</a> Reservas</h1>
        <?php 
            if($_SESSION['rol'] != "empleado"){
                echo '<button class="btnRegistro"><a href="registrarReserva.php">Registro de Reservas</a></button>';
            }
        ?>
        <table id="tablaHabitaciones">
            <thead>
                <th>ID Reserva</th>
                <th>Nombre Cliente</th>
                <th>Email Cliente</th> 
                <th>Habitacion</th>
                <th>Descripcion</th>
                <th>Cantidad personas</th>
                <th>Fecha Inicio</th>
                <th>Fecha Fin</th>
                <?php
                    if($_SESSION['rol'] != "empleado"){
                            echo '<th>Acciones</th>';
                    }
                ?>
            </thead>
            <tbody>
            <?php
            $reservaDAO = new ReservaDAO();
            $reservas = $reservaDAO->obtener_todas_reservas();
            $clienteDAO = new ClienteDAO();
            $habitacionDAO = new HabitacionDAO();

            foreach ($reservas as $reserva) {
                if($reserva){
                    $cliente = $clienteDAO->obtenerClientePorId($reserva->getIdCliente());
                    $habitacion = $habitacionDAO->obtenerHabitacionPorId($reserva->getNumeroHabitacion());
                    if($habitacion){
                        $descripcion = "Habitacion #".$reserva->getNumeroHabitacion(). " para " .$habitacion->getCapacidad(). " personas";
                    }else{
                        $descripcion = "Desconocido";
                    }
                    if($cliente){
                        $email = $cliente->getEmail();
                        $nombre = $cliente->getNombre();
                    }else{
                        $email = "Desconocido";
                        $nombre = "Desconocido";
                    }
                    echo '<tr>';
                    echo "<td>" . $reserva->getID() . "</td>";
                    echo "<td>" . $nombre . "</td>"; 
                    echo "<td>" . $email . "</td>";
                    echo "<td>" . $reserva->getNumeroHabitacion() . "</td>";
                    echo "<td>" . $descripcion . "</td>";
                    echo "<td>" . $reserva->getCantidadPersonas() . "</td>";
                    echo "<td>" . $reserva->getFechaInicio() . "</td>";
                    echo "<td>" . $reserva->getFechaFin() . "</td>";
                    if($_SESSION['rol'] != "empleado"){
                        echo '<td>';
                        echo '<button class="btnMostrar acciones">Mostrar</button>';
                        echo '<br><br>';
                        echo '<button class="btnModificar acciones">Modificar</button>';
                        echo '<br><br>';
                        echo '<button class="btnEliminar acciones">Eliminar</button>';
                        echo '</td>';
                    }
                    echo "</tr>";
                }
            }
            ?>
            </tbody>
        </table>
    </div>

    <div id="dialog" title="Detalles del Cliente" style="display:none;">
        <p><strong>ID Reserva:</strong> <span id="reservaID"></span></p>
        <p><strong>Nombre:</strong> <span id="clienteNombre"></span></p>
        <p><strong>Email:</strong> <span id="clienteEmail"></span></p>
        <p><strong>Descripcion:</strong> <span id="descripcionReserva"></span></p>
        <p><strong>Habitacion:</strong> <span id="habitacionReserva"></span></p>
        <p><strong>Fecha Inicio:</strong> <span id="fechaInicioReserva"></span></p>
        <p><strong>Fecha Fin:</strong> <span id="fechaFinReserva"></span></p>
        <p><strong>Cantidad Peronas:</strong> <span id="cantidadPersonas"></span></p>
        <button id='cerrarDialog'>Cerrar</button>
    </div>

    <div id="confirmarEliminarDialog" title="Confirmar Eliminación" style="display:none;">
        <p>¿Estás seguro de que deseas eliminar esta reserva?</p>
        <button id="cerrar">Cerrar</button>
        <button id="confirmar">Eliminar</button>
    </div>

    <div id="editDialog" style="display:none;">
        <h2>Editar Reserva</h2>
        <div class="dialogSpace">
            <p>Nueva cantidad personas:</p>
            <input type="text" id="eidtCantidad" placeholder="Nueva Cantidad Personas">
        </div>
        <button id="btnGuardarEdit">Guardar</button>
        <button id="btnCerrarEdit">Cerrar</button>
    </div>

</body>

<script>
    var mostrarBotones = document.querySelectorAll('button.btnMostrar');
    var modificarBotones = document.querySelectorAll('button.btnModificar');
    var eliminarBotones = document.querySelectorAll('button.btnEliminar');


    mostrarBotones.forEach(function (boton) {
        boton.addEventListener('click', function () {
            var fila = this.parentElement.parentElement;

            var idReserva = fila.querySelector('td:nth-child(1)').textContent;
            var nombreCliente = fila.querySelector('td:nth-child(2)').textContent;
            var emailCliente = fila.querySelector('td:nth-child(3)').textContent;
            var habitacion = fila.querySelector('td:nth-child(4)').textContent;
            var descripcion = fila.querySelector('td:nth-child(5)').textContent;
            var cantidadPersonas = fila.querySelector('td:nth-child(6)').textContent;
            var fechaInicio = fila.querySelector('td:nth-child(7)').textContent;
            var fechaFim = fila.querySelector('td:nth-child(8)').textContent;

            document.getElementById('reservaID').textContent = idReserva;
            document.getElementById('clienteNombre').textContent = nombreCliente;
            document.getElementById('clienteEmail').textContent = emailCliente;
            document.getElementById('descripcionReserva').textContent = descripcion;
            document.getElementById('habitacionReserva').textContent = habitacion;
            document.getElementById('fechaInicioReserva').textContent = fechaInicio;
            document.getElementById('fechaFinReserva').textContent = fechaFim;
            document.getElementById('cantidadPersonas').textContent = cantidadPersonas;

            document.getElementById('dialog').style.display = 'block';
        });
    });

    modificarBotones.forEach(function (boton) {
        boton.addEventListener('click', function () {
            var fila = this.parentElement.parentElement;
            var idReserva = fila.querySelector('td:nth-child(1)').textContent;
            var idHabitacion = fila.querySelector('td:nth-child(4)').textContent;
            var numeroPersonas = fila.querySelector('td:nth-child(6)').textContent;
            
            document.getElementById('eidtCantidad').value = numeroPersonas;

            document.getElementById('editDialog').style.display = 'block';

            

            document.getElementById('btnGuardarEdit').addEventListener('click', function () {
                
                var formData = new FormData();
                cantidadNueva = document.getElementById('eidtCantidad').value;

                formData.append("idReserva", idReserva);
                formData.append("idHabitacion", idHabitacion);
                formData.append("cantidadNueva", cantidadNueva);
                
                var xhr = new XMLHttpRequest();
                xhr.open("POST", "modificarReservaAJAX.php", true);
                xhr.onreadystatechange = function() {
                    if (xhr.readyState === 4 && xhr.status === 200) {
                        alert(xhr.responseText);
                        window.location.reload();
                    }
                };
                xhr.send(formData);
            });

        });
    });

    eliminarBotones.forEach(function (boton) {
        boton.addEventListener('click', function () {
            var fila = this.parentElement.parentElement;
            var idReserva = fila.querySelector('td:nth-child(1)').textContent;
            var idHabitacion = fila.querySelector('td:nth-child(4)').textContent;

            var confirmarEliminarDialog = document.getElementById('confirmarEliminarDialog');
            confirmarEliminarDialog.style.display = 'block';

            var formData = new FormData();
            formData.append("idReserva", idReserva);
            formData.append("idHabitacion", idHabitacion);

            document.getElementById('confirmar').addEventListener('click', function () {
                alert('Eliminar reserva con ID: ' + idReserva);
                var xhr = new XMLHttpRequest();
                xhr.open("POST", "eliminarReservaAJAX.php", true);
                xhr.onreadystatechange = function() {
                    if (xhr.readyState === 4 && xhr.status === 200) {
                        alert(xhr.responseText);
                        window.location.reload();
                    }
                };
                xhr.send(formData);
            });
        });
    });

    document.getElementById('cerrar').addEventListener('click', function () {
        document.getElementById('confirmarEliminarDialog').style.display = 'none';
    });

    document.getElementById('cerrarDialog').addEventListener('click', function () {
        document.getElementById('dialog').style.display = 'none';
    });

    document.getElementById('btnCerrarEdit').addEventListener('click', function () {
        document.getElementById('editDialog').style.display = 'none';
    });

</script>
</html>
