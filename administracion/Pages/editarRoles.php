<?php
session_start();
require '../../includes/requeriments_administracion.php';

if (!isset($_SESSION['id_usuario'])) {
    header('Location: ../../login.php');
    exit();
}else if($_SESSION['rol'] != 'administrador'){
    header('Location: ../../home.php');
    exit();
}

if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    session_destroy();
    header('Location: ../../login.php');
    exit();
}
?>

<!DOCTYPE html>
<html>
<head>
    <title>Módulo de Administración</title>
    <link rel="stylesheet" href="../../css/administracion_ver_usuarios.css">
</head>
<body>
    <div class="sidebar">
        <img class="image-logo"
         src="data:image/png;base64,"
         alt="Casa del Café">
        <a href="../../home.php">Ir al menú principal</a>
        <a href="verUsuarios.php">Ver Usuarios</a>
        <a href="editarRoles.php">Editar Roles</a>
        <a href="parametrosRoles.php">Editar parametros cargos</a>
        <form action="editarRoles.php" method="POST">
            <input type="submit" value="Cerrar Sesión" formnovalidate>
        </form>
    </div>
    <div class="content">
        <h1><a class="back-link" href="../index.php">&lt;Volver</a> Ver Usuarios</h1>
        <table id="tablaUsuarios">
            <thead>
                <th>ID</th>
                <th>Nombre y Apellido</th>
                <th>Rol</th>
                <th>Estado</th>
                <th>Acciones</th>
            </thead>
            <tbody>
            <?php
            $usuarioDAO = new UsuarioDAO();
            $usuarios = $usuarioDAO->obtener_todos_usuarios(); 

            foreach ($usuarios as $usuario) {
                $estadoUsuario = $usuario->get_usuario_activo() == 1? "ACTIVO": "SUSPENDIDO"; 
                echo '<tr data="' . $usuario->obtenerAtributosSeparadosPorPipe() . '">';
                echo "<td>" . $usuario->get_user_id() . "</td>";
                echo "<td>" . $usuario->get_nombre() . ' ' . $usuario->get_apellido() . "</td>";
                echo "<td>" . $usuario->obtener_rol() . "</td>";
                echo "<td>" . $estadoUsuario . "</td>";
                echo '<td>';
                echo '<button class="btnCambiarRol acciones">Cambiar Rol</button>';
                echo '<button class="btnModificarEstado acciones" data-user-id="' . $usuario->get_user_id() . '">Modificar Estado</button>';
                echo '</td>';
                echo "</tr>";
            }
            ?>
            </tbody>
        </table>
    </div>



    <div class="dialog-overlay">
        <div class="dialog-box">
            <h2>Cambiar Rol</h2>
            <p>Usuario ID: <span id="userId"></span></p>
            <div class="seguido" style="display: flex;">
                <label for="rol">Nuevo Rol:</label>
                <select id="rol" name="rol" style="width: 100%;">
                    <option value="administrador">Administrador</option>
                    <option value="coordinador">Coordinador</option>
                    <option value="empleado">Empleado</option>
                </select>
            </div>
            <button id="guardarCambio">Guardar Cambio</button>
            <button id="cerrarDialog">Cerrar</button>
        </div>
    </div>

    <div class="dialog-overlay-estado" style="display:none;">
        <div class="dialog-box-estado">
            <h2>Modificar Estado</h2>
            <p>Usuario ID: <span id="userIdEstado"></span></p>
            <div class="seguido" style="display: flex;">
                <label for="estado">Nuevo Estado:</label>
                <select id="estado" name="estado" style="width: 100%;">
                    <option value="1">ACTIVO</option>
                    <option value="0">SUSPENDIDO</option>
                </select>
            </div>
            <button id="guardarCambioEstado">Guardar Cambio de Estado</button>
            <button id="cerrarDialogEstado">Cerrar</button>
        </div>
    </div>

</body>

<script>

    document.addEventListener("DOMContentLoaded", function() {
        var table = document.getElementById("tablaUsuarios");
        var rows = table.getElementsByTagName("tr");

        for (var i = 1; i < rows.length; i++) {
            var row = rows[i];
            var data = row.getAttribute("data");
            var buttons = row.getElementsByTagName("button");

            buttons[0].addEventListener("click", function(data) {
                return function() {
                    showChangeRoleDialog(data);
                };
            }(data));

            buttons[1].addEventListener("click", function(data) {
                return function() {
                    showChangeEstadoDialog(data);
                };
            }(data));
        }
    });

    function showChangeEstadoDialog(data) {
        var dialogOverlayEstado = document.querySelector(".dialog-overlay-estado");
        var dialogBoxEstado = document.querySelector(".dialog-box-estado");
        var userIdSpanEstado = dialogBoxEstado.querySelector("#userIdEstado");
        var estadoSelect = dialogBoxEstado.querySelector("#estado");
        var guardarCambioEstadoButton = dialogBoxEstado.querySelector("#guardarCambioEstado");
        var cerrarDialogEstadoButton = dialogBoxEstado.querySelector("#cerrarDialogEstado");

        var selectedUserIdEstado = data.split("|")[0];

        userIdSpanEstado.textContent = selectedUserIdEstado;
        dialogOverlayEstado.style.display = "block";

        guardarCambioEstadoButton.addEventListener("click", function() {
            var newEstado = estadoSelect.value;
            if (selectedUserIdEstado && newEstado) {
                var xhr = new XMLHttpRequest();
                xhr.open("POST", "cambiarEstado.php", true);
                xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
                xhr.onreadystatechange = function() {
                    if (xhr.readyState === 4 && xhr.status === 200) {
                        alert(xhr.responseText);
                        dialogOverlayEstado.style.display = "none";
                        window.location.reload();
                    }
                };
                xhr.send("user_id=" + selectedUserIdEstado + "&new_estado=" + newEstado);
            }
        });

        cerrarDialogEstadoButton.addEventListener("click", function() {
            dialogOverlayEstado.style.display = "none";
        });
    }

    
    document.addEventListener("DOMContentLoaded", function() {
        var table = document.getElementById("tablaUsuarios");
        var rows = table.getElementsByTagName("tr");
        
        for (var i = 1; i < rows.length; i++) {
            var row = rows[i];
            var data = row.getAttribute("data");
            var buttons = row.getElementsByTagName("button");

            buttons[0].addEventListener("click", function(data) {
                return function() {
                    showChangeRoleDialog(data);
                };
            }(data));
        }
    });

    function showChangeRoleDialog(data) {
        var dialogOverlay = document.querySelector(".dialog-overlay");
        var dialogBox = document.querySelector(".dialog-box");
        var userIdSpan = dialogBox.querySelector("#userId");
        var rolSelect = dialogBox.querySelector("#rol");
        var guardarCambioButton = dialogBox.querySelector("#guardarCambio");
        var cerrarDialogButton = dialogBox.querySelector("#cerrarDialog");

        var selectedUserId = data.split("|")[0];

        userIdSpan.textContent = selectedUserId;
        dialogOverlay.style.display = "block";

        guardarCambioButton.addEventListener("click", function() {
            var newRole = rolSelect.value;
            if (selectedUserId && newRole) {
                var xhr = new XMLHttpRequest();
                xhr.open("POST", "cambiarRol.php", true);
                xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
                xhr.onreadystatechange = function() {
                    if (xhr.readyState === 4 && xhr.status === 200) {
                        alert(xhr.responseText);
                        dialogOverlay.style.display = "none";
                        window.location.reload();
                    }
                };
                xhr.send("user_id=" + selectedUserId + "& new_role=" + newRole);
            }
        });

        cerrarDialogButton.addEventListener("click", function() {
            dialogOverlay.style.display = "none";
        });
    }

</script>

</html>
