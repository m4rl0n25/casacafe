...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    if(isset($_GET['correo']) and isset($_GET['contrasena'])) {
        $correo = $_GET['correo']; 
        $contrasena = $_GET['contrasena']; 

        $usuarioDAO = new UsuarioDAO();
        $idUsuario = $usuarioDAO->iniciar_sesion($correo, $contrasena);
        
        if($idUsuario){
            $rolDAO = new RolDAO();
            if($idUsuario[0] != 'Usuario suspendido'){
                $_SESSION['id_usuario'] = $idUsuario[0];
                $_SESSION['nombre_usuario'] = $idUsuario[2];
                $_SESSION['correo'] = $correo;
                $_SESSION['contraseña'] = $contrasena;
                $_SESSION['rol'] = $rolDAO->getRolIdPorNombre($idUsuario[1]);
                
                header('Location: index.php'); // redirige sin enviar salida antes
                exit();
            }else{
                echo "<script>alert('Su usuario se encuentra suspendido');</script>";
            }
        }else{
...
<meta charset="UTF-8">
    ...
    <title>login</title>
    <link rel="stylesheet" href="css/administracion_login.css">
        <img class="imagen-agrandada"
        ...
        <body>
          <div>
            ...
            <form action="login.php" method="GET">
                ...
                <input type="text" placeholder="Ingrese su nombre de usuario" name="correo" required>
