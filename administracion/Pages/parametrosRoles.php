<?php
session_start();
require '../../includes/requeriments_administracion.php';

if (!isset($_SESSION['id_usuario'])) {
    header('Location: ../../login.php');
    exit();
}else if($_SESSION['rol'] != 'administrador'){
    header('Location: ../../home.php');
    exit();
}

if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    session_destroy();
    header('Location: ../../login.php');
    exit();
}
?>

<!DOCTYPE html>
<html>
<head>
    <title>Módulo de Administración</title>
    <link rel="stylesheet" href="../../css/administracion_ver_usuarios.css">
</head>
<body>
    <div class="sidebar">
        <img class="image-logo"
         src="data:image/png;base64,"
         alt="Casa del Café">
        <a href="../../home.php">Ir al menú principal</a>
        <a href="verUsuarios.php">Ver Usuarios</a>
        <a href="editarRoles.php">Editar Roles</a>
        <a href="parametrosRoles.php">Editar parametros cargos</a>
        <form action="parametrosRoles.php" method="POST">
            <input type="submit" value="Cerrar Sesión" formnovalidate>
        </form>
    </div>
    <div class="content">
        <h1><a class="back-link" href="../index.php">&lt;Volver</a> Ver Cargos</h1>
        <table id="tablaCargos">
            <thead>
                <th>Cargo</th>
                <th>Salario</th>
                <th>Acciones</th>
            </thead>
            <tbody>
            <?php
            // Usar la función para obtener los cargos y salarios desde la base de datos
            $rolDAO = new RolDAO();
            $cargos = $rolDAO->obtenerCargosYSalarios();

            if ($cargos) {
                foreach ($cargos as $cargoSalario) {
                    echo '<tr data="' . $cargoSalario['cargo'] . '|' . $cargoSalario['salario'] . '">';
                    echo "<td>" . $cargoSalario['cargo'] . "</td>";
                    echo "<td>" . $cargoSalario['salario'] . "</td>";
                    echo '<td>';
                    echo '<button class="btnModificarSalario acciones">Modificar Salario</button>';
                    echo '</td>';
                    echo "</tr>";
                }
            }
            ?>
            </tbody>
        </table>
    </div>
</body>

<script>
    
    document.addEventListener("DOMContentLoaded", function() {
        var table = document.getElementById("tablaCargos");
        var rows = table.getElementsByTagName("tr");
        
        for (var i = 1; i < rows.length; i++) {
            var row = rows[i];
            var data = row.getAttribute("data");
            var buttons = row.getElementsByTagName("button");

            buttons[0].addEventListener("click", function(data) {
                return function() {
                    showModifySalaryDialog(data);
                };
            }(data));
        }
    });

    function showModifySalaryDialog(data) {
        var dialogOverlay = document.querySelector(".dialog-overlay");
        var dialogBox = document.querySelector(".dialog-box");
        var cargoSpan = dialogBox.querySelector("#cargo");
        var salarioInput = dialogBox.querySelector("#salario");
        var modificarSalarioButton = dialogBox.querySelector("#modificarSalario");
        var cerrarDialogButton = dialogBox.querySelector("#cerrarDialog");

        var cargoSalario = data.split("|");
        var selectedSalario = cargoSalario[1];

        cargoSpan.textContent = cargoSalario[0];
        salarioInput.value = selectedSalario;
        dialogOverlay.style.display = "block";

        modificarSalarioButton.addEventListener("click", function() {
            var newSalario = salarioInput.value;
            if (cargoSalario[0] && newSalario) {
                var selectedCargo = cargoSpan.textContent;
                var xhr = new XMLHttpRequest();
                var params = "cargo=" + cargoSalario[0] + "& salario=" + newSalario;

                xhr.open("POST", "modificarSalario.php", true);
                xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
                xhr.onreadystatechange = function() {
                    if (xhr.readyState === 4 && xhr.status === 200) {
                        alert(xhr.responseText); 
                        dialogOverlay.style.display = "none";
                        window.location.reload();
                    }
                };
                xhr.send(params);
            }
        });


        cerrarDialogButton.addEventListener("click", function() {
            dialogOverlay.style.display = "none";
        });
    }

</script>

<div class="dialog-overlay">
    <div class="dialog-box">
        <h2>Modificar Salario</h2>
        <p>Cargo: <span id="cargo"></span></p>
        <div class="seguido" style="display: flex;">
            <label for="salario">Nuevo Salario:</label>
            <input type="number" id="salario" name="salario" min="0">
        </div>
        <button id="modificarSalario">Modificar Salario</button>
        <button id="cerrarDialog">Cerrar</button>
    </div>
</div>

</html>
